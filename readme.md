...
1. **Read JavaScript source** – the program first decides where the code comes from:
...
2. **Create a temporary file** – a unique file is created in `/tmp` (Linux/macOS) or a similar location, using `mkstemp()` for security.
3. **Write code to the file** – the JS code is written into that temporary file.
4. **Execute Node.js** – the program runs `node tempfile.js` and captures both stdout and stderr using `popen()`.
5. **Return output** – the captured output is printed to the terminal (stdout).
6. **Clean up** – the temporary file is deleted.
